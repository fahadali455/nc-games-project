...
exports.getReviewsQuery = (category, sort_by = "created_at", order = "DESC") => {
    ...
    const validCategories = ["euro-game", "dexterity", "social deduction", "children's games", undefined];
    ...
    else if(!validCategories.includes(category) || !validSortColumn.includes(sort_by) || !validOrder.includes(order)){
        return "400";
    }

    const categoryQuery = category !== undefined ? `WHERE category = '${category.replaceAll("'", "''")}' ` : ``;
